...
STATIC_WSTRING(logname, "\\systemroot\\temp\\TL-SRV%c%u-%02u-%02u.log");

NTSTATUS CLogFile::Init()
{
	WCHAR lpFileName[256];

	if (0 >= swprintf_s(lpFileName, _countof(lpFileName), logname, 0, 0, 0, 0))
	{
...
NTSTATUS CLogFile::Init(_In_ PTIME_FIELDS tf)
{
	...
	if (0 >= swprintf_s(lpFileName, _countof(lpFileName), logname, '\\', tf->Year, tf->Month, tf->Day))
	{
		return STATUS_INTERNAL_ERROR;
	}
