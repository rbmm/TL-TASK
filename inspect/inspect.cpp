...
-- */
{
    ...
    if (layerData != NULL)
    {
        ...
        //
		switch (FwpsQueryPacketInjectionState(gInjectionHandle, (const NET_BUFFER_LIST*)layerData, NULL))
		{
		case FWPS_PACKET_INJECTED_BY_SELF:
		case FWPS_PACKET_PREVIOUSLY_INJECTED_BY_SELF:
...
-- */
{
    ...
    if (packetDirection == FWP_DIRECTION_INBOUND)
    {
        ...
        else
        {
            ...
            //
            FWPS_PACKET_LIST_INFORMATION packetInfo = { };
...
{
	if (!filterKey) filterKey = &emptyG;

	DbgPrint("%hs(%x {%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X} %016I64X)\r\n", 
		__FUNCTION__, notifyType,
		filterKey->Data1, filterKey->Data2, filterKey->Data3, 
		filterKey->Data4[0], filterKey->Data4[1], filterKey->Data4[2], filterKey->Data4[3], 
		filterKey->Data4[4], filterKey->Data4[5], filterKey->Data4[6], filterKey->Data4[7],
		filter->filterId
		);
...
-- */
{
    ...
    while (!gDriverUnloading)
    {
        ...
        if (IsListEmpty(&gConnList) && IsListEmpty(&gPacketQueue) && !gDriverUnloading)
        {
