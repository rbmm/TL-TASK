...
)
{
    ...
    packet->protocol = (IPPROTO)inFixedValues->incomingValue[protocolIndex].value.uint8;
...
)
{
    ...
    pendedPacket = (TL_INSPECT_PENDED_PACKET*)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    if (layerData)
    {
        pendedPacket->netBufferList = (NET_BUFFER_LIST*)layerData;
    ...
    pendedPacket->compartmentId = (COMPARTMENT_ID)inMetaValues->compartmentId;
        ...
        (layerData != NULL))
    {
            ...
            FWPS_METADATA_FIELD_TRANSPORT_CONTROL_DATA))
        {
            ...
            pendedPacket->controlData = (WSACMSGHDR*)ExAllocatePool2(
                POOL_FLAG_NON_PAGED,
...
IsTrafficPermitted()
{
    ...
    return 0 > ZwQueryValueKey(gParametersKey, &valueName, KeyValuePartialInformationAlign64, &kvpi, sizeof(kvpi), &rcb) ||
        REG_DWORD != kvpi.Type || sizeof(ULONG) != kvpi.DataLength || !*(ULONG*)kvpi.Data;
}
